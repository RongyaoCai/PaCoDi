...
def save_visualization(ori_data, generated_data, save_path, analysis='pca', compare=3000):
    ...
    plt.legend()
    save_file = os.path.join(save_path, f"vis_{analysis}.png")
...
def plot_4_samples(ori_data, fake_data, save_path):
    ...
    for i in range(4):
        ...
        ax[i].legend()
    plt.tight_layout()
    save_file = os.path.join(save_path, "sample_comparison.png")
    plt.savefig(save_file)
    plt.close()
...
if __name__ == "__main__":
    ...
    csv_file = "final_evaluation_summary_uncond.csv"
