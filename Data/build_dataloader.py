...
from Utils.Data_utils.T2S_dataset import T2SDataset
...
def build_dataloader(config, args=None):
    ...
    config['dataloader']['train_dataset']['params']['output_dir'] = args.savesample_dir
...
def build_dataloader_text(config, args, period='train'):

    Data_name = config['dataloader']['train_dataset']['params']['name']

    is_train = (period == 'train')
    batch_size = config['dataloader']['batch_size'] if is_train else config['dataloader'].get('sample_size', 16)

    dataset = T2SDataset(
        name=Data_name,
        data_root = f'./Data/datasets/TSFragment-600K/',
        period=period
    )
