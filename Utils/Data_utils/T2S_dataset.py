...
class T2SDataset(Dataset):
    def __init__(
        ...
        self.data, self.text, self.embedding,self.scaler = self.read_data(self.name, self.csv_data)
        ...
        self.global_mean = np.mean(train_data, axis=(0, 1))
        self.global_std = np.std(train_data, axis=(0, 1)) + 1e-6  # 防止除以0

        self.global_mean = self.global_mean.astype(np.float32)
        self.global_std = self.global_std.astype(np.float32)
    ...
    def unnormalize(self, data):
        ...
        data_2d = data.reshape(-1, self.var_num)
        physical_data = self.scaler.inverse_transform(data_2d)
        return physical_data.reshape(orig_shape)
    ...
    def read_data(name, json_root_data):
        ...
        if 'TSL' in name.split('_'):
            ...
            scaler = MinMaxScaler()
            scaler.fit(time_series.reshape(-1, 1))
            time_series = scaler.transform(time_series.reshape(-1, 1)).reshape(time_series.shape)
        ...
        return time_series, text, embedding, scaler
