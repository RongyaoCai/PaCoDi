...
from Utils.io_utils import normalize_to_neg_one_to_one, unnormalize_to_zero_to_one
...
class CustomDataset(Dataset):
    ...
    def __getsamples(self, data, proportion, seed):
        ...
        if self.save2npy:
            if 1 - proportion > 0:
                np.save(os.path.join(self.dir, "ground_truth_test.npy"), self.unnormalize(test_data))
            np.save(os.path.join(self.dir, "ground_truth_train.npy"), self.unnormalize(train_data))
            if self.auto_norm:
                if 1 - proportion > 0:
                    np.save(os.path.join(self.dir, "norm_truth_test.npy"), unnormalize_to_zero_to_one(test_data))
                np.save(os.path.join(self.dir, "norm_truth_train.npy"), unnormalize_to_zero_to_one(train_data))
            else:
                if 1 - proportion > 0:
                    np.save(os.path.join(self.dir, "norm_truth_test.npy"), test_data)
                np.save(os.path.join(self.dir, "norm_truth_train.npy"), train_data)
